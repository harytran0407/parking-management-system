...
export default function Header() {
  ...
  const pageTitles = {
    ...
    "/staff/profile": "Staff Profile Settings",
  };
  ...
  const getFallbackTitle = () => {
    if (user?.role === "ParkingStaff" || user?.role === "STAFF") return "Operator Dashboard";
  ...
  const targetProfileRoute = user?.role === "ParkingStaff" || user?.role === "STAFF" ? "/staff/profile" : "/user/profile";
  ...
  // ==========================================
  // 🛠️ BỘ LỌC HIỂN THỊ DANH TÍNH TỐI ƯU (UX PROTECTION)
  // ==========================================
  // Hàm này quét mọi ngóc ngách dữ liệu để lấy bằng được tên người dùng
  const getUserDisplayName = () => {
    if (!user) return "Guest User";

    // 1. Ưu tiên các trường tên đầy đủ chuẩn hóa
    if (user.full_name) return user.full_name;
    if (user.fullName) return user.fullName;
    if (user.name) return user.name;

    // 2. Fallback sang tài khoản định danh hệ thống
    if (user.username) return user.username;

    // 3. Nếu không có tên, tự động bẻ khóa lấy phần chữ trước ký tự @ của Email
    if (user.email) {
      return user.email.split("@")[0];
    }

    // 4. Giải pháp cuối cùng dựa theo nhóm quyền thực tế
    return user.role === "ParkingStaff" || user.role === "STAFF" ? "Parking Operator" : "Parking Driver";
  };

  const displayName = getUserDisplayName();

  // Tạo chữ cái đại diện Avatar (Lấy ký tự đầu tiên của tên hiển thị)
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className={`w-10 h-10 rounded-full text-white font-bold flex items-center justify-center shadow-md transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900 overflow-hidden ${
              user?.role === "ParkingStaff" || user?.role === "STAFF" ? "bg-indigo-600 hover:bg-indigo-700" : "bg-blue-600 hover:bg-blue-700"
            } ${isDropdownOpen ? "ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-slate-900" : ""}`}>
            {user?.avatar ? <img src={user.avatar} alt="User Avatar" className="w-full h-full object-cover" /> : initial}
          </button>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-3.5 w-60 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200 origin-top-right">
              <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/40">
                {/* 🔴 ĐÃ ĐƯỢC THAY THẾ: Hiển thị biến danh tính thông minh bảo vệ UI */}
                <p className="text-sm font-black text-slate-900 dark:text-white truncate leading-tight">{displayName}</p>
                <div className="mt-1.5">
                  <span
                    className={`text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider ${
                      user?.role === "ParkingStaff" || user?.role === "STAFF"
