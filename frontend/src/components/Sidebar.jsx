...
=======
      ...
      >
        ...
        >
          <div
            className={`flex items-center justify-between w-full min-w-0 ${isCollapsed ? "lg:hidden" : "flex"}`}
          >
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-8 h-8 bg-blue-600 text-white rounded-xl flex items-center justify-center font-black text-base shadow-md shrink-0">
                S
              </div>
              <div className="whitespace-nowrap">
...
=======
          <div
            className={`w-10 h-10 items-center justify-center relative group/logo ${isCollapsed ? "hidden lg:flex" : "hidden"}`}
          >
            <button
              onClick={() => setIsCollapsed(false)}
              className="w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-200 focus:outline-none"
            >
...
=======
              ...
              >
                <div className="flex items-center gap-3.5 z-10 min-w-0">
                  <div
                    className={`transition-transform duration-200 group-hover:scale-105 shrink-0 ${isActive ? "text-blue-600 dark:text-blue-400" : "text-slate-400"}`}
                  >
                    {item.icon}
                  </div>

                  <span
                    className={`leading-none whitespace-nowrap ${isCollapsed ? "lg:hidden" : "block"}`}
                  >
                    {item.label}
                  </span>
                </div>
                ...
                {item.badge && (
                  <div
                    className={`items-center gap-2 z-10 ${isCollapsed ? "lg:hidden flex" : "flex"}`}
                  >
...
=======
                {/* 🚀 CHANGED: Applied soft light/dark background, subtle border, matching texts and glass blur filter */}
                <div
                  className={`absolute left-full ml-3 bg-slate-100/80 dark:bg-slate-900/80 text-slate-800 dark:text-slate-200 text-xs font-extrabold px-3.5 py-2 rounded-xl shadow-xl border border-slate-200/60 dark:border-slate-800/80 backdrop-blur-md opacity-0 pointer-events-none group-hover:opacity-100 transition-all duration-200 z-[9999] translate-x-2 group-hover:translate-x-0 whitespace-nowrap
                    ${isCollapsed ? "hidden lg:block" : "hidden"}
                  `}
                >
                  {item.label}
                  {item.badge && (
                    <span className="ml-2 bg-blue-600 text-white text-[9px] font-black font-mono px-1.5 py-0.5 rounded shadow-sm">
                      {item.badge}
                    </span>
                  )}
                </div>
                ...
                {isActive && (
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1.5 h-1/2 bg-blue-600 dark:bg-blue-500 rounded-l-full shadow-sm" />
                )}
>>>>>>> origin/main
              </Link>
            );
...
=======
        {/* ECOSYSTEM ATTACHED COPYRIGHT FOOTER */}
        <div className="p-4 border-t border-slate-100 dark:border-slate-800/60 text-center shrink-0">
          <p className="text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest font-mono">
            <span className={isCollapsed ? "lg:hidden" : ""}>
              Smartpark v1.0
            </span>
            <span className={`hidden ${isCollapsed ? "lg:inline" : ""}`}>
              v1.0
            </span>
          </p>
>>>>>>> origin/main
