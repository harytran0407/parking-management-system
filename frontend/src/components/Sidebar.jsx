...
export default function Sidebar({ navigationItems = [], isCollapsed, setIsCollapsed }) {
  ...
  const checkIsActive = (path) => {
    ...
    if (targetPath === "/staff" || targetPath === "/user" || targetPath === "/admin" || targetPath === "") {
      ...
      <aside
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800/60 text-center shrink-0">
          <p className="text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest font-mono">
            <span className={isCollapsed ? "lg:hidden" : ""}>Smartpark v1.0</span>
            <span className={`hidden ${isCollapsed ? "lg:inline" : ""}`}>v1.0</span>
          </p>
        </div>
      </aside>
