...
import { Car, Bike, ArrowLeft, CheckCircle, MapPin, X, Clock, CreditCard, CheckCircle2, Info, PlusCircle, RotateCcw } from "lucide-react";
...
export default function BookSlot() {
  ...
  const filteredVehicles = useMemo(() => {
    return userVehicles.filter((v) => v.vehicle_type === bookingData.vehicleType);
  ...
  useEffect(() => {
    if (timeData.startTime && timeData.endTime) {
      ...
      if (end > start) {
        ...
        setTotalPrice(Math.ceil(hours) * (bookingData.vehicleType === "car" ? 20000 : 5000));
      } else {
        setTotalPrice(0);
      }
    }
  ...
  const handleSelectSlot = (slot) => {
    ...
    if (slot.status.toLowerCase() !== "available") {
      console.log("⛔ Chặn kích hoạt: Ô này không ở trạng thái trống. Trạng thái thực tế:", slot.status);
      return;
    }
  ...
  const selectedVehiclePlate = useMemo(() => {
    if (isAddingNewVehicle && newPlateNumber) return newPlateNumber.toUpperCase();
    return userVehicles.find((v) => v.id === Number(bookingData.vehicleId))?.plate_number || "N/A";
  }, [userVehicles, bookingData.vehicleId, isAddingNewVehicle, newPlateNumber]);

  const isFormValid = useMemo(() => {
    const isTimeFilled = timeData.startTime && timeData.endTime && totalPrice > 0;
    return isAddingNewVehicle ? newPlateNumber.trim().length >= 5 && isTimeFilled : bookingData.vehicleId !== "" && isTimeFilled;
  }, [isAddingNewVehicle, bookingData.vehicleId, newPlateNumber, timeData, totalPrice]);
    ...
    <div className="animate-fade-in h-[calc(100vh-8rem)] flex flex-col relative">
      <div className="mb-6 flex items-center gap-4">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
          Select Location{" "}
          <span className="text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/50 px-3 py-1 rounded-lg text-lg capitalize">({bookingData.vehicleType})</span>
        </h2>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-hidden">
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-sm rounded-3xl">
            ...
            <ul className="space-y-4">
              {[
                ...
                return (
                  <li key={item.key} className="flex items-center justify-between font-semibold text-sm">
                    <div className="flex items-center gap-3">
                      <span className={`w-3.5 h-3.5 rounded-md border ${currentStyle.badge}`}></span>
                      <span className="text-slate-700 dark:text-slate-300">{item.label}</span>
                    </div>
                    <span className={`font-bold font-mono text-base ${currentStyle.text}`}>{item.count || 0}</span>
                  </li>
                );
              })}
            </ul>
          ...
          <div className="bg-gradient-to-br from-blue-600 to-blue-800 dark:from-slate-800 dark:to-slate-900 rounded-3xl p-6 text-white relative overflow-hidden shadow-sm">
            ...
            <p className="text-4xl font-black text-white mb-1">{occupancyRatePercentage}%</p>
            <p className="text-sm text-blue-100 dark:text-slate-300">{stats.available} slots remaining</p>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden relative flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                <MapPin size={20} className="text-blue-600" /> Slot {bookingData.slotName} - Floor {bookingData.floorId}
              </h2>
              {modalPhase === "form" && (
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1.5">
                      <label className="text-sm font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-2">
                        {bookingData.vehicleType === "car" ? <Car size={16} /> : <Bike size={16} />} Vehicle Information:
                      </label>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    {!isAddingNewVehicle && filteredVehicles.length === 0 && (
                      <p className="text-[11px] text-amber-500 mt-1 font-medium">💡 No saved vehicles found. Click "+ New Vehicle" above to register!</p>
                    )}
                  </div>
              ...
              {modalPhase === "payment" && (
                <div className="text-center animate-fade-in">
                  ...
                  <div className="bg-white p-4 rounded-2xl inline-block border-2 border-slate-200 mb-6 shadow-sm">
                    <img src={qrCodeData} alt="Banking QR" className="w-48 h-48" />
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3 mb-6">
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300">
                      Amount: <span className="text-blue-600 dark:text-blue-400 font-bold text-lg">{DEPOSIT_FEE.toLocaleString()} VND</span>
                    </p>
                  </div>
                  <button onClick={handlePaymentSuccess} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3.5 rounded-xl shadow-lg">
              ...
              {modalPhase === "success" && (
                <div className="text-center animate-fade-in">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
                    Here is your e-ticket for vehicle <span className="font-mono font-bold text-slate-700 dark:text-slate-300">{selectedVehiclePlate}</span>.
                  </p>
                  ...
                  <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-xl p-3 mb-6 text-left flex gap-3 items-start">
                    ...
                    <p className="text-xs font-medium text-amber-700 dark:text-amber-400 leading-relaxed">
                      Important Note: Security camera will scan your plate at the gate. If failure occurs, present this QR ticket to the staff.
                    </p>
                  </div>
                  <button
                    onClick={() => {
