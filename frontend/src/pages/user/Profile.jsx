...
export default function Profile() {
  ...
  // 1. STATE LƯU THÔNG TIN USER (Sau này sẽ fetch từ API)
  ...
  // Edit avatar by uploading file
    const handleImageChange = (e) =>{
        ...
        if(file) {
            ...
            reader.onloadend = () =>{
                setProfile({...profile,avatar:reader.result});// udpate avatar to state
            };
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className={`flex-1 bg-transparent text-slate-800 dark:text-white font-medium py-2 border-b-2 transition-colors focus:outline-none ${
                    isEditing.name ? 'border-blue-500' : 'border-slate-200 dark:border-slate-700'
                  }`}
                />
                <button 
                ...
                >
                  {isEditing.name ? <Check size={18} className="text-green-500" /> : <Edit2 size={18} />}
                </button>
              </div>
            </div>
