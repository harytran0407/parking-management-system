...
import { MapPin, Clock, Car, Bike, AlertTriangle, Info, CheckCircle2, XCircle, Calendar } from 'lucide-react';
...
export default function UserDashboard() {
  ...
  const buildingInfo = {
    ...
    mapUrl: "https://maps.google.com/maps?q=Saigon%20Hi-Tech%20Park,%20Ho%20Chi%20Minh&t=&z=15&ie=UTF8&iwloc=&output=embed",
    imageUrl: "https://i2-prod.leicestermercury.co.uk/article9542323.ece/ALTERNATES/s1200/0_GettyImages-1458811529.jpg",
    is24_7: false,
  ...
  const totalAvailable = buildingInfo.availableSlots.car + buildingInfo.availableSlots.motorbike;
            ...
            <div className="flex flex-col space-y-6">
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{buildingInfo.weekdayHours}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Weekend:</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{buildingInfo.weekendHours}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <div className={`flex-1 p-2.5 rounded-xl border flex flex-col items-center justify-center ${buildingInfo.availableSlots.car > 0 ? 'bg-emerald-50/50 dark:bg-emerald-950/10 border-emerald-100 dark:border-emerald-900/30' : 'bg-red-50/50 dark:bg-red-950/10 border-red-100 dark:border-red-900/30'}`}>
                    <Car size={16} className={buildingInfo.availableSlots.car > 0 ? 'text-emerald-500' : 'text-red-500'} />
                    <span className={`text-base font-bold mt-0.5 ${buildingInfo.availableSlots.car > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                      {buildingInfo.availableSlots.car > 0 ? buildingInfo.availableSlots.car : 'FULL'}
                    </span>
                    <span className="text-[9px] text-slate-400 uppercase">Cars</span>
                  </div>
                  <div className={`flex-1 p-2.5 rounded-xl border flex flex-col items-center justify-center ${buildingInfo.availableSlots.motorbike > 0 ? 'bg-emerald-50/50 dark:bg-emerald-950/10 border-emerald-100 dark:border-emerald-900/30' : 'bg-red-50/50 dark:bg-red-950/10 border-red-100 dark:border-red-900/30'}`}>
                    <Bike size={16} className={buildingInfo.availableSlots.motorbike > 0 ? 'text-emerald-500' : 'text-red-500'} />
                    <span className={`text-base font-bold mt-0.5 ${buildingInfo.availableSlots.motorbike > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                      {buildingInfo.availableSlots.motorbike > 0 ? buildingInfo.availableSlots.motorbike : 'FULL'}
                    </span>
                    <span className="text-[9px] text-slate-400 uppercase">Bikes</span>
                  </div>
                </div>
              </div>
