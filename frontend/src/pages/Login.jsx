    ...
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="backdrop-blur-md bg-white border border-white/20 shadow-2xl rounded-xl p-8 relative">
          ...
          {error && <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm font-medium">{error}</div>}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-slate-800 mb-1">{t[language].emailOrPhone}</label>
              <div className="relative">
                <Mail className="absolute text-slate-600 left-3 top-1/2 -translate-y-1/2 text-slate-450 w-5 h-5" />
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder={t[language].emailOrPhonePlaceholder}
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50/80 border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500 transition"
                  required
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-sm font-semibold text-slate-800">{t[language].password}</label>
              ...
              <div className="relative">
                <Lock className="absolute text-slate-600 left-3 top-1/2 -translate-y-1/2 text-slate-450 w-5 h-5" />
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-slate-50/80 border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500 transition"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-450 hover:text-slate-700 focus:outline-none"
            ...
            <div className="flex justify-center py-2 bg-slate-50 border border-slate-200 rounded-lg">
              {RECAPTCHA_SITE_KEY ? (
            ...
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-2.5 px-4 rounded-lg transition duration-200 focus:outline-none mt-2 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <button
            ...
            disabled={submitLoading || googleLoading}
            className="w-full flex items-center justify-center gap-3 bg-slate-50 hover:bg-slate-100 border border-slate-300 text-slate-800 font-semibold py-2.5 px-4 rounded-lg transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
            <img src={googleIcon} alt="Google" className="w-5 h-5" />
            {googleLoading ? t[language].processing : t[language].googleBtn}
          </button>

          <p className="text-center text-sm text-slate-600 mt-6 mb-2">
